\chapter{Measuring the material radioactivity}

Essential for this work is to know how much radioactivity is expected to be found in the optical modules. One method for measuring isotope activities is gamma spectroscopy. This can be done with a scintillator or a semiconductor detector. For measurements where the isotopes are not known, usually the latter is used, as it yields a much higher energy resolution and therefore the gamma energies can be better differentiated and classified. Hence, this thesis will focus only on spectroscopy with semiconductor detectors.
\par In 2016 measurements were done at the University of Alberta, Canada, at a low-level counting setup for, among others, a Vitrovex and a Benthos sample. The results of these measurements needed correction factors that were calculated in this work. Also, for the sake of better statistics, more measurements were carried out in the scope of this thesis at the University of Münster.
\par At the beginning of this chapter, a small introduction to Gamma spectroscopy is given (\ref{sec:gammaspec}). The second part (\ref{subsec:gammAlberta}) is devoted to the analysis of the measurements of Alberta and at the last part the measurements done in Münster are explained and evaluated (\ref{subsec:gammMünster}). An extensive description of gamma spectroscopy and semiconductor detectors can be found in \cite{Leo1994} and \cite{knoll2010radiation}, and a more specific description for low-level counting in \cite{Zvara1994}.
...
\vspace{-10pt}
\end{wrapfigure}
With gamma spectroscopy, it is possible to calculate unknown concentration of radionuclides by evaluating the energies and intensities of gammas emitted by decays in a sample. These gammas interact with the detector medium releasing ionising electrons by the photoelectric effect, Compton scattering or pair production. In the case of a semiconductor detector, which is based on a p-n junction, the ionising electrons produce electron-hole pairs in a number that is proportional to the absorbed energy. These charge carriers are then collected and amplified, for then being read out as a voltage pulse. This is digitised by a MCA regarding its amplitude. \par A typical pulse-height spectrum is shown in figure \ref{fig:gammafuncresp}. At the left side of the plot, one can see the full energy peak (FEP) produced when the gamma energy is totally absorbed. At lower energies of the FEP, one can find the Compton continuum, that is measured when the gamma scatters in the detector but then escapes without being fully absorbed. The calibration from channel to energy can be done by measuring a known radioactive source and relating the centre of its FEPs to the gammas energy. In this manner, one can later estimate the gamma-ray energy of a sample by determining the position of the FEPs in the spectrum, which ultimately leads to the identification of the decaying isotope. The concentrations of this radionuclide can be then calculated by evaluating the area under its FEPs.
\par For a total number number of counts (net peak area) $S_{\rm{0}}$ from a FEP corresponding to the energy $E_{\rm{0}}$, the activity of the emitter isotope $A_{\rm{0}}$ can be calculated with 
\begin{equation}
\label{formula:activity}
    A_{\rm{0}} = \dfrac{S_{\rm{0}}}{t_{\rm{m}} \cdot \varepsilon (E_{\rm{0}}) \cdot I_{\rm{0}}},
\end{equation}
where $t_{m}$ is the time of measurement,  $\varepsilon (E_{\rm{0}})$ the absolute detection efficiency for FEPs with energy $E_{\rm{0}}$ and $I_{\rm{0}}$ the gamma-ray intensity (probability of emission of the particular gamma-ray per decay). The efficiency $\varepsilon (E)$ will depend on the geometry of the setup, and on the energy of the gamma quantum, as the interaction probability for the photoelectric effect is energy dependent. It can be calculated using Monte Carlo simulations, taking into account the geometries and materials of the detector and the source, and on the distance between them.
...
\vspace{-10pt}
\end{wrapfigure}
\par Since in a normal gamma spectrum there are many FEPs with different energies, the peaks are superimposed on a background formed by the Compton continuum coming from the more energetic gamma-rays. Therefore, for the estimation of the net area peak one has to subtract the area of the Compton base from the total peak area. There are different approaches for this evaluation \cite{prospect}, but so long the spectrum does not have poor counting statistics, there is no much difference between the results of these methods. In this thesis, it is going to be assumed a linear continuum under the FEP, which is described in \cite{prospect}. Here, the average background counts per channel is calculated immediately before and after the peak. This value is then subtracted from the peak. If the FEP is found across N bins, 
...
\par Not all the measured FEPs will be coming from radionuclides in the sample. There is also a contribution from activities contained in the detector and materials surrounding the detector, and radiation from cosmic rays. Therefore it is important to reduce these external sources by shielding the detector with high atomic number materials like lead. Anyway, it is necessary to make a background measurement (that is, without any sample in the detector complex), since it is not possible to completely eliminate the external background. The gamma spectra have to be then corrected with this background measurement.
\par The levels of background radiation of the detector determines the sensitivity of the system. If for a background measurement of time $t_0$, there is $B_0$ counts at a specific FEP, then the uncertainty of this measurement will be $\sigma_{\rm{B_0}}=\sqrt{B_0}$, as it follows Poisson statistics. For a signal coming from a sample to be statistically valid, it has to produce at least $>\sqrt{B_0}$ counts, which means there is a lower boundary $L(t_0,E_0)$ for the measurable activities, which will be dependent on the detection system and the measured time. Following \autoref{formula:activity} this can be determined to be
\begin{equation}
    L(t_0,E_0) = \dfrac{\sigma_{\rm{B}}(E_0)}{t_{\rm{0}} \cdot \varepsilon (E_{\rm{0}}) \cdot I_{\rm{0}}}
\end{equation}
for a FEP corresponding to an energy $E_0$. \par The background measurements of both, the setup in Alberta and the one in Münster, can be found in figure \ref{fig:backgroundgamma}. It is noticeable that for most energies, the background of the system in Alberta is almost one order of magnitude fainter. This makes it more suitable for finding gamma-rays with low intensities and isotopes with small activities.  Therefore, the isotope identification will be done from the measurements done in Canda, which are presented in the next section. Nevertheless, it will be shown that the setup of Münster is sensible enough for measuring most of the isotopes in the sample, allowing to calculate activities with similar relative uncertainties.
\begin{figure}[H]
    \centering
...
The Vitrovex sample measured in Alberta has a cylindrical shape and weights $\SI{575\pm5}{g}$, which was provided by the company. The sample of Benthos glass is a fragment coming from a half vessel of an IceCube DOM, and weights  $\SI{341(5)}{g}$. The measurement of the first sample lasted $\SI{159741}{s}$ and from the latter $\SI{264998}{s}$. The results from the Vitrovex sample can be seen in figure \ref{fig:LandscapeFigure}. For the identification of most of the peaks, the software Prospect Genie 2000 was used, which has a gamma-ray energy library. It can be seen that all gamma-rays can be identified coming from only 12 different isotopes. These, excepting for \ch{^{40}K}, are part of one of the three natural decay chains: \ch{^{235}U} , \ch{^{238}U} and $\ch{^{232}Th}$. The radionuclides from these series with their most intensive gamma-emissions can be found in table \ref{tab:allisotopes}.

\begin{table}[H]
\centering
\caption{Most energetic gamma-ray energy of each isotope from the $\ch{^{238}U}$ ,$\ch{^{235}U}$ and $\ch{^{232}Th}$ chain. In bold and italic are the isotopes that were measured in both, the spectrum of the Benthos and Vitrovex sample. \\
...
\begin{sidewaysfigure}[]
    ...
    \caption{Gamma spectrum of the measurement done in the University of Alberta with the Vitrovex sample and the background.}
    \label{fig:LandscapeFigure}
\end{sidewaysfigure}



For the estimation of the absolute detection efficiency, a Geant4 simulation provided by Pawel Mekarski from the University of Alberta was used. In this code, the detector geometry is defined. In the scope of this work, the geometry of the samples was incorporated to the program \footnote{Picutres of the samples and their geometry in Geant4 can be found in appendix REF!.}. The efficiency is calculated by simulating $N_T$ gamma-rays coming isotropically from the sample and saving the deposited energy in the active volume of the germanium crystal. The result of this is a histogram similar to the response function seen in figure \ref{fig:gammafuncresp}. Then, integrating the FEP of this histogram, one becomes the number of gamma-rays $N_D$ that were totally absorbed in the detector, which ultimately leads to the efficiency $\varepsilon = \dfrac{N_D}{N_T}$.
\par Once the efficiency is simulated and calculated for every gamma-ray energy of the found identified isotopes, it is possible to determine the decay rate of the radionuclides. For this, only the most intense gamma-rays were used, which did not overlap with other lines. In table \ref{table:actCanada} the resulting mass-specific activity $\frac{A}{m}$ are provided for each isotope. Here the total activity of the sample calculated with \autoref{formula:activity} is divided by its mass $m$ for better comparison between specimens of the same material.
\par In nature, the three decay chains are approximately in secular equilibrium, so long the soil has been undisturbed for a long time \cite{9781498769297}. This means, it is to be expected that all isotopes listed in table \ref{tab:allisotopes} should have the same decay rate within its decay series. Consequently, once the activity of an isotope from a chain is known, one can calculate the expected number of detected gamma-rays coming from a specific isotope, by solving for $S_0$ in the \autoref{formula:activity}. This was done for all undetected isotopes from the series. Four from the $\ch{^{235}U}$- and one from $\ch{^{232}Th}$-chain, should have been detected in the measurement if the assumption of secular equilibrium were to be correct. Nevertheless, all their gamma-rays that were intense enough to be detected, had a similar energy of another FEP with higher activity. For example, the line from $\ch{^{223}Ra}$ and $\ch{^{219}Rn}$ with an energy of $\SI{269.5}{keV}$ and $\SI{271.2}{keV}$, from the $\ch{^{235}U}$-chain, lie in between a line from $\ch{^{228}Ac}$ with an energy of $\SI{270.2}{keV}$ from the $\ch{^{232}Th}$-series, which has a higher activity. Hence, both isotopes could have been detected, but can not be distinguished within the detector resolution. This is also the case for the $\SI{269.5}{keV}$-gamma-ray $\ch{^{224}Ra}$ and the  $\SI{89.95}{keV}$ from $\ch{^{231}Th}$ (see figure \ref{fig:LandscapeFigure}).

\par It is noticeable that the activities of isotopes of a single series do not agree within their error, but are similar. Hence, the assumption of secular equilibrium is only an approximation. Nevertheless, it is going to be assumed that all isotopes of the series have the same activity, using the average of measured activities, which also can be found in table \ref{table:actCanada}. In the case of the $\ch{^{235}U}$-series, only one isotope in the chain was measured, namely $\ch{^{235}U}$, and therefore its activity is assumed for the rest of radionuclides of the series.

\begin{table}[H]
\centering
\caption{Mass-specific activities from the samples measured in the University of Alberta. For the case of the $\ch{^{238}U}$- and $\ch{^{232}Th}$ series, the average of its isotopes activities is also given.}
...
\par It is to notice that the Benthos sample has a far lower relative amount of radioisotopes than the Vitrovex one, especially of $\ch{^{40}K}$, which is more than twelve times higher in the latter. This would imply that choosing the Benthos glass would yield lower amounts of dark rates coming from luminescence, assuming that both glasses have the same scintillation properties. This raises the question, if the "cleanliness" of the glass samples are an intrinsic property of the brand, or if there is a high variance between specimens of the same material. To check up on this, new measurements were done at the University of Münster. Also, specimens of the two gel brands, Wacker and QSI, were made and measured. The next section presents the results of these measurements. 
...
\par The Vitrovex sample measured in Canada was one of a set of three that were delivered from the company. These samples were available for measurements in Münster, together with two half pressure vessels of the same shape, one made from Benthos and one from Vitrovex glass. The latter is an old specimen of the early 2000s. As this samples have unalike geometries, they will have very different absolute detection efficiencies. While the smaller cylindrical samples can be placed near the germanium detector, the half vessel will use a larger volume, reducing the solid angle from where the gammas will be detected. The efficiency was therefore simulated preparatory to the measurement so that the measurement times could be estimated in such a way, that the measured spectra had similar statistical uncertainties. For this, a Geant4 code with the geometry of Münster's detection system provided by Dr Volker Hannen was integrated into the software used in the last section. 
\par The results can be found in figure \ref{fig:efficiency}. The detection efficiency for the cylindrical samples is, as expected, almost one order of magnitude larger than the one for the half vessels. However, the mass of the small samples is also approximately one order of magnitude smaller, meaning that a similar measuring time for all specimens should deliver similar statistical uncertainties. The evaluation of the peaks was the same as in the last section. The calculated activities are presented in table \ref{tab:actMue} and the spectra can be found in Appendix \textcolor{red}{Blah!}. 
